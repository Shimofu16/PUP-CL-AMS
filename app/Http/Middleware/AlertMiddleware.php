...
class AlertMiddleware
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($request->session()->has('infoAlert')) {
            Alert::info('Info', $request->session()->get('infoAlert'))->autoClose(5000)->animation('animate__zoomIn', 'animate__zoomOut')->timerProgressBar();
        }
        ...
        if ($request->session()->has('errorToast')) {
            toast()->error('SYSTEM MESSAGE', $request->session()->get('errorToast'))->autoClose(5000)->animation('animate__fadeInRight', 'animate__fadeOutRight')->timerProgressBar();
        }
        if ($request->session()->has('warningToast')) {
            toast()->warning('SYSTEM MESSAGE', $request->session()->get('warningToast'))->autoClose(5000)->animation('animate__fadeInRight', 'animate__fadeOutRight')->timerProgressBar();
        }
        if ($request->session()->has('infoToast')) {
            toast()->info('Info', $request->session()->get('infoToast'))->autoClose(5000)->animation('animate__fadeInRight', 'animate__fadeOutRight')->timerProgressBar();
        }
        if ($request->session()->has('questionToast')) {
            toast()->question('SYSTEM MESSAGE', $request->session()->get('questionToast'))->autoClose(5000)->animation('animate__fadeInRight', 'animate__fadeOutRight')->timerProgressBar();
        }
